...
<body style="background-color:#6C7990;">
    ...
    <div id="wrap">
        ...
        if (isset($_GET['id']) && !empty($_GET['id'])) {
            ...
            if ($search = $conn->query("SELECT hash, active FROM user WHERE hash='" . $hash . "' AND active IS NULL")) {
                $count = $search->num_rows;
            }
            if ($count > 0) {       
                
                $conn->query("UPDATE user SET active='1' WHERE hash='".$hash."' AND active IS NULL");
        ...
        <?php
            ...
            ?>
          <?php  include_once dirname(__FILE__).'/sendcomic.php'; ?>
