    ...
    <div id="wrap">
        ...
        if (isset($_POST['name']) && !empty($_POST['name']) and isset($_POST['email']) && !empty($_POST['email'])) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($result = $conn->query("SELECT * FROM user WHERE email = '$email' and active is NOT NULL")) {
                    $count = $result->num_rows;
                }
                if ($count == 0) {
                    //send email if new email

                    if ($conn->query("SELECT * FROM user WHERE email = '$email' and active is NULL")->num_rows == 1) {
                        $conn->query("UPDATE user set name = '$name', hash = '$hash' where email = '$email'");
                        $msg = 'Verifification email has been resent, Name updated successfully!';
                        $color = 'color:green;';
                    } else {
                        mysqli_query($conn, "INSERT IGNORE INTO user (name, email, hash) VALUES('" . $name . "', '" . $email . "', '" . $hash . "') ");
                    ...
                    $mail->Body    .= $verify.$_SERVER['HTTP_HOST'].'/assignment/verify.php?id='. $hash."\r\n";
                    if (!$mail->send()) {
